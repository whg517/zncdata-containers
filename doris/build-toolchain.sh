...
##########################################################################################
# Define versions
echo 'export LDB_TOOLCHAIN_VERSION=0.21' >> /etc/profile
echo 'export PRODUCT_VERSION=2.1.7-rc03' >> /etc/profile
echo 'export JAVA_VERSION=8' >> /etc/profile
source /etc/profile
...
echo 'export JAVA_HOME=/usr/lib/jvm/temurin-${JAVA_VERSION}-jdk' >> /etc/profile
echo 'export JAVA_VERSION=${JAVA_VERSION}' >> /etc/profile
...
echo 'export PATH="/opt/gettext/bin:$PATH"' >> /etc/profile
...
echo 'export PATH="/opt/ldb-toolchain/bin:$PATH"' >> /etc/profile
...
push /build/doris
./thirdparty/build-thirdparty.sh -j $(nproc)
...
nohup ./build.sh -j $(nproc) > /tmp/build.log 2>&1 &
